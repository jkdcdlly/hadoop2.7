...
public class StandbyCheckpointer {
  ...
  private void doCheckpoint() throws InterruptedException, IOException {
    ...
    // See HDFS-4816
    //将保存的检查点上载回活动的，在单独的线程中执行此操作，以避免阻塞到活动的转换，见HDFS-4816
  ...
  private class CheckpointerThread extends Thread {
    ...
    private void doWork() {
      ...
      // Reset checkpoint time so that we don't always checkpoint on startup.
      ...
      while (shouldRun) {
        ...
        try {
          ...
          final long uncheckpointed = countUncheckpointedTxns(); // 差多少条数据 1000000
          final long secsSinceLast = (now - lastCheckpointTime) / 1000; // 差多少时间 3600s
