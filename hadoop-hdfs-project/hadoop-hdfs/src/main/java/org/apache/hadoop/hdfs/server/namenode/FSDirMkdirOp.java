...
class FSDirMkdirOp {
  static HdfsFileStatus mkdirs(FSNamesystem fsn, String src, PermissionStatus permissions, boolean createParent) throws IOException {
    ...
    FSPermissionChecker pc = fsd.getPermissionChecker();// 获得权限检查器
    byte[][] pathComponents = FSDirectory.getPathComponentsForReservedPath(src); // 把路径按照split('/')切分，然后转成byte数组
    fsd.writeLock();
    try {
      src = fsd.resolvePath(pc, src, pathComponents);
      INodesInPath iip = fsd.getINodesInPath4Write(src);// 获取与文件/目录相关联的INode。
      // 检查权限的
      ...
      if (lastINode == null) {
        ...
        //验证是否有足够的索引节点。这至多是一种启发式操作，因为mkdirs()操作可能需要创建多个inode。
        ...
        if (length > 1) {
          ...
          // TODO 创建目录的核心代码 通过向所有祖先目录添加隐式的u+wx权限，确保用户可以遍历路径
