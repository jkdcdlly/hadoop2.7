...
class BPServiceActor implements Runnable {
  ...
  private void connectToNNAndHandshake() throws IOException {
    // get NN proxy
    // TODO 注意：这里获得了 NameNode 的代理
    //  dn.connectToNN(nnAddr) 返回一个 DatanodeProtocolClientSideTranslatorPB 对象，
    //  这个对象里包装了 NameNode 的代理
    bpNamenode = dn.connectToNN(nnAddr);
    // First phase of the handshake with NN - get the namespace
    // info.
    // TODO 与 NN 握手 第一阶段 - 获得 namespace 信息
  ...
  void start() {
    ...
    bpThread.start(); // TODO 启动 BPServiceActor ，下一步是 run
  }
  ...
  private void offerService() throws Exception {
    LOG.info("For namenode " + nnAddr + " using"
        + " DELETEORT_INTERVAL of " + dnConf.deleteReportInterval + " msec "
      ...
      try {
        ...
        if (startTime - lastHeartbeat >= dnConf.heartBeatInterval) {
          ...
          if (!dn.areHeartbeatsDisabledForTests()) {
            HeartbeatResponse resp = sendHeartBeat();// TODO 发送心跳
            assert resp != null;
  ...
  void register(NamespaceInfo nsInfo) throws IOException {
    ...
    bpRegistration = bpos.createRegistration(); // TODO 创建 DataNode 注册信息
  ...
  public void run() {
    ...
    try {
      while (true) { // TODO 不断尝试注册
        // init stuff
        try {
          // setup storage
          // TODO 链接 NameNode 并且握手
          ...
          if (shouldRetryInit()) { // 如果可重试的
            // Retry until all namenode's of BPOS failed initialization
            // 重试，直到BPOS的所有namenode的初始化失败
