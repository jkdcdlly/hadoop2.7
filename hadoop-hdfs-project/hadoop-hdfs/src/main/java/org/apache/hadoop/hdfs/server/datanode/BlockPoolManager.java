...
class BlockPoolManager {
  ...
  private void doRefreshNamenodes(
    ...
    synchronized (this) {
      // Step 1. For each of the new nameservices, figure out whether it's an update of the set of NNs for an existing NS, or an entirely new nameservice.
      // TODO 步骤1。检查每个新的 namenservices 是新增的还是更新的 ;联邦情况有多个 namenservices，否则就一个 namenservices
      for (String nameserviceId : addrMap.keySet()) {
        if (bpByNameserviceId.containsKey(nameserviceId)) { // 是更新的
          toRefresh.add(nameserviceId);
        } else {
          toAdd.add(nameserviceId); // 是新增的
        }
      }
      // Step 2. Any nameservices we currently have but are no longer present need to be removed.
      // TODO 步骤2。任何我们目前拥有但不再存在的命名服务都需要删除。
      ...
      // TODO 注册和心跳都在这里 start all
      startAll();
    }

    // Step 4. Shut down old nameservices. This happens outside of the synchronized(this) lock since they need to call back to .remove() from another thread
    // TODO 步骤4. 关闭旧的namesservices。这发生在synchronized(This)锁之外，因为它们需要从另一个线程回调.remove()
    if (!toRemove.isEmpty()) {
