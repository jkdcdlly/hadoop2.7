...
public class DatanodeManager {
  ...
  DatanodeManager(final BlockManager blockManager, final Namesystem namesystem,
    ...
    final long heartbeatIntervalSeconds = conf.getLong(
        ...
        DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_DEFAULT); // 3 seconds
  ...
  void activate(final Configuration conf) {
    decomManager.activate(conf); // TODO 启动 decommission monitor 线程
    heartbeatManager.activate(conf);// TODO 启动心跳监控线程
  }
  ...
  boolean isDatanodeDead(DatanodeDescriptor node) {
    // 当前时间-最后监控时间 超过 10分30秒， 认为 DataNode 死亡
  ...
  void addDatanode(final DatanodeDescriptor node) {
    ...
    synchronized(datanodeMap) {
      // TODO 向 datanodeMap 中添加 datanode 数据
      host2DatanodeMap.remove(datanodeMap.put(node.getDatanodeUuid(), node));
    }
    // TODO 向 networktopology 中添加 datanode 数据
    networktopology.add(node); // may throw InvalidTopologyException
    // TODO 向 host2DatanodeMap 中添加 datanode 数据
    host2DatanodeMap.add(node);

    // 检查以上的内存结构，是否有datanode信息
    // 完成注册
   ...
   * 翻译：向 namenode 注册给定的 datanode。注:给定的注册被改变并返回给 datanode。
    ...
    try {
      ...
      if (nodeS != null) {
        ...
        try {
          ...
          // TODO also treat the registration message as a heartbeat
          // TODO 也将注册消息视为心跳
      ...
      try {
        ...
        // TODO 还可以将注册消息视为心跳，无需更新其时间戳，因为它是在创建描述符时完成的
        // TODO datanode 心跳
